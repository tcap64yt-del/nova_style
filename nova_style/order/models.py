...
class OrderAddress(models.Model):
    ...
    order = models.ForeignKey(
        Orders, on_delete=models.CASCADE, related_name="addresses"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["order", "address_type"], name="unique_order_address_type"
            )
        ]
...
class OrderCancellation(models.Model):
    order = models.OneToOneField(
        Orders, on_delete=models.CASCADE, related_name="cancellation"
    )
...
class OrderItemCancellation(models.Model):
    order_item = models.ForeignKey(
        OrderItems, on_delete=models.SET_NULL, null=True, related_name="cancellation"
    )
    quantity = models.PositiveIntegerField()
    reason = models.CharField(max_length=255)
...
class OrderReturns(models.Model):
    ...
    order = models.OneToOneField(
        Orders, on_delete=models.CASCADE, related_name="returns"
    )
    reason = models.CharField(max_length=255)
...
class OrderItemReturn(models.Model):
    ...
    order_item = models.ForeignKey(
        OrderItems, on_delete=models.CASCADE, related_name="returns"
    )
    quantity = models.PositiveIntegerField()
    reason = models.CharField(max_length=255)
    description = models.TextField(blank=True)
