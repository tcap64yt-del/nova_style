...
def validate_coupon(user,code,subtotal):
    ...
    if coupon.min_order_amount:
        if subtotal < coupon.min_order_amount:
            errors["coupon"] = (f"Minimum order amount is "f"₹{coupon.min_order_amount}.")
            return None, Decimal("0"), errors
...
def calculate_item_refund(order_item, quantity):
    ...
    item_gross = (Decimal(str(order_item.unit_amount)) * Decimal(str(order_item.quantity)))
    ...
    already_cancelled = (order_item.cancellation.aggregate(total=Sum("quantity") )["total"] or 0)

    already_returned = ( order_item.returns.filter( status="returned").aggregate(total=Sum("quantity"))["total"] or 0)
    ...
    cumulative_refund = (per_quantity * Decimal(str(new_processed))).quantize( Decimal("0.01"), rounding=ROUND_HALF_UP)

    previous_refund = ( per_quantity * Decimal(str(already_processed))).quantize( Decimal("0.01"), rounding=ROUND_HALF_UP)
...
def apply_coupon(request):
    ...
    coupon, discount, errors = validate_coupon(
        request.user,code,subtotal)
    if errors:
...
def checkout (request,variant_id=None):
    ...
    if coupon_code:
        coupon, discount_amount, coupon_errors = validate_coupon(request.user,coupon_code,subtotal)
    ...
    if request.method=="POST":
        ...
        if coupon_code:
            ...
            if coupon_errors:
                return render(request, "checkout/checkout.html",{"details": profile,"items": items,"buy_now": buy_now,"subtotal": subtotal,"default_address": default_address,"addresses": addersses,"variant_id": variant_id,"discount_amount": Decimal("0"),"final_amount": subtotal,"coupon_code": "","coupon_error": coupon_errors["coupon"],"checkout_errors": [],"available_coupons":available_coupons,"wallet":wallet})

        final_amount = (subtotal - discount_amount).quantize(Decimal("0.01"))
        checkout_errors = []
        payment_method = request.POST.get("payment_method")
        if payment_method  =="wallet" and wallet.balance < final_amount:
            checkout_errors = [{ "name": "Wallet", "message": ( f"Insufficient wallet balance. " f"Available balance: ₹{wallet.balance:.2f}. " f"Required amount: ₹{final_amount:.2f}." ) }]
            return render(request,"checkout/checkout.html",{"details":profile,"items":items,"buy_now":bool(buy_now),"subtotal":subtotal,"default_address":default_address,"addresses":addersses,"variant_id": variant_id,"discount_amount": discount_amount,"final_amount": final_amount,"coupon_code": coupon_code,"coupon_error": coupon_error,"checkout_errors": checkout_errors,"available_coupons":available_coupons,"wallet":wallet})
        ...
        if not buy_now:
            ...
            if variant.stock < 1:
                checkout_errors.append({"name": variant.product.name,"color": variant.color,"size": variant.size,"stock": variant.stock,})
        if checkout_errors:
            return render(request,"checkout/checkout.html",{"details": profile,"items": items,"buy_now": bool(buy_now),"subtotal": subtotal,"default_address": default_address,"addresses": addersses,"checkout_errors": checkout_errors,"variant_id": variant_id,"available_coupons":available_coupons,"wallet":wallet,"balance_after_payment":balance_after_payment})

        if request.POST.get("action") == "add_address":
            

            address_data={"name" : request.POST.get("name", "").strip(),
            ...
            Addresses.objects.create(user=request.user,name=address_data["name"],phone=address_data["phone"],address=address_data["address"],district=address_data["district"],state=address_data["state"],country=address_data["country"],postal_code=address_data["postal_code"],is_default=is_default)
            return redirect("checkout")
        errors={}
    ...
    "postal_code": request.POST.get("shippingPostal", ""),}
        ...
        if errors: 
            return render(request,"checkout/checkout.html",{"errors":errors,"details":profile,"items":items,"buy_now":buy_now,"subtotal":subtotal,"default_address":default_address,"addresses":addersses,"variant_id": variant_id,"available_coupons":available_coupons,"wallet":wallet,"balance_after_payment":balance_after_payment})
        if buy_now:
            if variant.stock < 1:
                 checkout_errors = [{"name": variant.product.name,"color": variant.color,"size": variant.size,"stock": variant.stock,}]

                 return render( request,"checkout/checkout.html",{"details": profile,"items": items,"buy_now": True,"subtotal": subtotal,"default_address": default_address,"addresses": addersses,"checkout_errors": checkout_errors,"variant_id": variant_id,"available_coupons":available_coupons,"wallet":wallet,"balance_after_payment":balance_after_payment},)



        
        order = Orders.objects.create(user=request.user,subtotal=subtotal,final_amount=final_amount,discount_amount=discount_amount,coupon_code=coupon_code if coupon_code else None)
     
        OrderTrack.objects.create(order=order,status="pending",)
        OrderAddress.objects.create(order=order,address_type="shipping",name=shipping_data["name"],phone=shipping_data["phone"],address=shipping_data["address"],state=shipping_data["state"],district=shipping_data["district"],country=shipping_data["country"],postal_code=shipping_data["postal_code"],)    
        if billing_same:
            OrderAddress.objects.create(order=order,address_type="billing",name=shipping_data["name"],phone=shipping_data["phone"],address=shipping_data["address"],state=shipping_data["state"],district=shipping_data["district"],country=shipping_data["country"],postal_code=shipping_data["postal_code"],)
        else:
            OrderAddress.objects.create(order=order,address_type="billing",name=billing_data["name"],phone=billing_data["phone"],address=billing_data["address"],state=billing_data["state"],district=billing_data["district"],country=billing_data["country"],postal_code=billing_data["postal_code"],)
        if payment_method == "cod":
            ...
            wallet.save(update_fields=['balance'])
            WalletTransaction.objects.create(wallet=wallet,amount=final_amount,type="debit",status="completed")
        ...
        return redirect("success",order_id=order.id)
    return render(request,"checkout/checkout.html",{"details":profile,"items":items,"buy_now":bool(buy_now),"subtotal":subtotal,"default_address":default_address,"addresses":addersses,"variant_id": variant_id,"discount_amount": discount_amount,"final_amount": final_amount,"coupon_code": coupon_code,"coupon_error": coupon_error,"available_coupons":available_coupons,"wallet":wallet,"balance_after_payment":balance_after_payment})
...
def order_details(request,order_id):
    STATUSS = ["pending","order placed","shipped","out for delivery","delivered","return pending","returned","rejected"]
    ...
    for item in items: 
        ...
        item.returned_return = (item.returns.filter(status="returned").aggregate(total=Sum("quantity"))["total"] or 0)
        item.available_return = (item.quantity- item.pending_return- item.returned_return)
    return render(request,"order/order_details.html",{"details":profile,"order":order,"items":items,"shipping":shipping,"billing":billing,"tracking":tracking,"steps": steps,"total":total,"current_index": current_index,"status_list": STATUSS,"progress":progress,"cancel_index":cancel_index,"tracking_map":tracking_map,"payment": payment,"discount_amount": order.discount_amount or Decimal("0.00"),"coupon_code": order.coupon_code,})
...
def return_product(request,item_id):
    ...
    if request.method == "POST":
        ...
        reason = request.POST.get("reason")
        description = request.POST.get("description")
        cancelled_qty =(order_item.cancellation.aggregate(total=Sum("quantity"))['total'] or 0)
        pending_qty=(order_item.returns.filter(status="pending").aggregate(total=Sum("quantity"))['total'] or 0)
        returned_qty =(order_item.returns.filter(status="returned").aggregate(total=Sum('quantity'))['total'] or 0)
        ...
        OrderItemReturn.objects.create(order_item=order_item,quantity=quantity,reason=reason,description=description,status='pending')
...
def verify_razorpay_payment(request):
    ...
    try:
        ...
        if payment.status == "paid":
            ...
            CartItem.objects.filter(cart__user=request.user,variant__in=[item.variant for item in order_items]).delete()
...
def razorpay_payment_failed(request):
    ...
    try:
        ...
        print("FAILED PAYMENT ERROR:", repr(e))

        return JsonResponse({
            "status": "error",
            "message": "Unable to save failed payment."
        }, status=500)
...
def retry_payment(request, order_id):
    ...
    Payment.objects.create(order=order,payment_method="razorpay",amount=amount,currency="INR",status="failed",razorpay_order_id=razorpay_order["id"],)
    order.payment_status = "failed"
    order.save(update_fields=["payment_status"])
