...
from .models import Category, ProductVariant, Review
...
def product_list(request):
    ...
    variants = (
        ...
        .annotate(
            category_offer=Coalesce(
                F("product__category__offer"), Value(0), output_field=DecimalField()
            ),
            variant_offer=Case(
                When(
                    offer__gt=0,
                    then=Case(
                        ...
                        When(end_date__isnull=False, end_date__lt=today, then=Value(0)),
                        default=F("offer"),
        ...
        .annotate(best_offer=Greatest(F("category_offer"), F("variant_offer")))
        .annotate(
            final_price=ExpressionWrapper(
                F("price") - (F("price") * F("best_offer") / Value(100)),
    ...
    if request.user.is_authenticated:
        ...
        if wishlist:
            wishlist_variants = list(
                wishlist.wishlist_items.values_list("variant_id", flat=True)
            )
    return render(
        request,
        "product/product_list.html",
        {
            ...
            "details": details,
            "wishlist_variant_ids": wishlist_variants,
            "show_search": True,
        },
    )
...
def product_details(request, variant_id):
    ...
    related_products = (
        ProductVariant.objects.filter(
            product__category=category, is_active=True, product__is_active=True
        )
