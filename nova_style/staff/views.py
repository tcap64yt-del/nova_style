...
def validate_coupon(data, coupon_id=None):
    ...
    if not code:
        ...
        errors["code"] = (
            ...
            "numbers, underscore or hyphen, atleast 3 letter."
        )
        ...
        try:
            ...
            if min_order_amount <= 0:
                errors["min_order_amount"] = (
                    "Minimumm purchase amount must greater than zero."
                )
        except (InvalidOperation, ValueError):
    ...
    cleaned_data = {
        ...
        "discount_value": (
            discount_value if isinstance(discount_value, Decimal) else None
        ),
...
def user_management(request):
    ...
    return render(
        request,
        "staff/admin_user_management.html",
        {
            "users": page_obj,
            "total": total,
            "sort": sort,
            "search": search,
            "page_obj": page_obj,
            "active_page": "admin_user_management",
        },
    )
...
def category_management(request):
    ...
    return render(
        request,
        "staff/admin_category_management.html",
        {
            "categories": page_obj,
            ...
            "search": search,
            "sort": sort,
            "active_page": "admin_category_management",
        },
    )
...
def new_category(request):
    ...
    if request.method == "POST":
                ...
                if offer_value < 0 or offer_value > 100:
                    ...
                    if start_date:
                        ...
                        if start_date < str(today):
                            errors["start_date"] = (
                                "start date cannot be in the before today"
                            )
                        if start_date and end_date:
                            if end_date < start_date:
        ...
        Category.objects.create(
            name=name,
            offer=offer if request.POST.get("offer") else None,
...
def product_management(request):
    ...
    sort = request.GET.get("sort", "all")
    products = Products.objects.annotate(count=Count("variants")).prefetch_related(
        "variants"
    )
    ...
    return render(
        request,
        "staff/admin_product_management.html",
        {
            "products": page_obj,
            "page_obj": page_obj,
            "search": search,
            "sort": sort,
            "active_page": "admin_product_management",
        },
    )
...
def add_product(request):
    ...
    if request.method == "POST":
        ...
        variants = []
        variant_count = max(
            len(prices), len(sizes), len(colors), len(stocks), len(statuses)
        )
            ...
            variants.append(
                {
                    ...
                    "offer": offer[i] if i < len(offer) else "",
                    "start_date": start_date[i] if i < len(start_date) else "",
                    "end_date": end_date[i] if i < len(end_date) else "",
                    "status": statuses[i] if i < len(statuses) else "true",
                    "errors": {},
                }
            )
        ...
        for i in range(len(prices)):
            ...
            variant = ProductVariant.objects.create(
                ...
                color=colors[i],
                price=prices[i],
                offer=offer[i] if offer[i] else None,
                start_date=start_date[i] if start_date[i] else None,
                end_date=end_date[i] if end_date[i] else None,
                stock=stocks[i],
                is_active=(statuses[i] == "true"),
            )
            variant_images = request.FILES.getlist(f"images_{i}[]")

            for i, image in enumerate(variant_images):
                ProductImage.objects.create(
                    variant=variant, image=image, is_primary=(i == 0)
                )
...
def edit_product(request, product_id):
    products = get_object_or_404(
        Products.objects.prefetch_related("variants__images"), id=product_id
    )
    ...
    if request.method == "POST":
        ...
        if delete_image_ids:
            ...
            affected_variant_ids = list(
                deleted_images.values_list("variant_id", flat=True)
            )
        ...
        for i in range(variant_count):
            ...
            variants.append(
                {
                    "id": variant_ids[i] if i < len(variant_ids) else "",
                    "index": i,
                    "price": prices[i] if i < len(prices) else "",
                    "size": sizes[i] if i < len(sizes) else "",
                    "color": colors[i] if i < len(colors) else "",
        ...
        if not product_name:
            ...
            Products.objects.filter(name__iexact=product_name)
        ...
        for i in range(len(prices)):
            ...
            if variant_id:
                ...
                duplicate = ProductVariant.objects.filter(
                    product=products, size=sizes[i], color=colors[i]
                ...
                variant = ProductVariant.objects.create(
                    ...
                    stock=stocks[i],
                    offer=offer[i] or 0,
                    start_date=start_date[i] or None,
                    end_date=end_date[i] or None,
                    is_active=(statuses[i] == "true"),
                )
            ...
            for img_index, image in enumerate(upload_images):
                ProductImage.objects.create(
                    variant=variant,
                    image=image,
                    is_primary=(existing_images == 0 and img_index == 0),
                )
...
def order_management(request):
    ...
    if search:
        orders = orders.filter(
            ...
            | Q(addresses__name__icontains=search)
        ).distinct()
    if status:
    ...
    return render(
        request,
        "staff/order_management.html",
        {
            "orders": page_obj,
            "page_obj": page_obj,
            "search": search,
            "active_page": "admin_order_management",
            "status": status,
        },
    )
...
def admin_order_detail(request, order_id):
    ...
    order = get_object_or_404(
        Orders.objects.select_related("user").prefetch_related("payments"), id=order_id
    )
    items = (
    ...
    return render(
        request,
        "staff/admin_order_details.html",
        {
            "order": order,
            "payment": payment,
            "items": items,
...
def admin_order_returns(request):
    ...
    order_returns = OrderReturns.objects.select_related(
        "order", "order__user"
    ).prefetch_related(
        Prefetch(
            "order__addresses",
            queryset=OrderAddress.objects.filter(address_type="shipping"),
            to_attr="shipping",
        )
    )
    item_returns = OrderItemReturn.objects.select_related(
        "order_item",
        "order_item__order",
    ...
    if search:
        order_returns = order_returns.filter(
            Q(order__addresses__name__icontains=search)
        ).distinct()
        item_returns = item_returns.filter(
            Q(order_item__order__addresses__name__icontains=search)
        ).distinct()
    ...
    returns = sorted(
        chain(order_returns, item_returns),
    ...
    return render(
        request,
        "staff/admin_order_returns.html",
        {
            "returns": returns,
            "search": search,
            "status": status,
...
def return_details(request, return_type, return_id):
    ...
    if return_type == "order":
        ...
        return_obj = get_object_or_404(
            OrderReturns.objects.select_related(
                "order", "order__user"
            ).prefetch_related(
                Prefetch(
                    "order__addresses",
                    queryset=OrderAddress.objects.filter(address_type="shipping"),
                    to_attr="shipping_address",
                ),
                "order__items__variant__product",
                "order__items__variant__images",
            ),
            id=return_id,
        )
        ...
        if return_obj.status == "pending":
            for item in items:
                ...
                returned_qty = (
                    item.returns.filter(status="returned").aggregate(
                        total=Sum("quantity")
                    )["total"]
                    or 0
                )
        ...
        refund_amount = calculate_item_refund(
            return_obj.order_item, return_obj.quantity
        )
            ...
            if return_type == "order":
                ...
                for item in order.items.select_related("variant"):
                    ...
                    ProductVariant.objects.filter(id=item.variant.id).update(
                        stock=F("stock") + remaining_qty
                    )

                    if (
                        cancelled_qty + already_returned_qty + remaining_qty
                        >= item.quantity
                    ):
...
def admin_coupon_management(request):
    ...
    return render(
        request,
        "staff/admin_coupon_management.html",
        {
            "active_page": "admin_coupon_management",
            "coupons": page_obj,
            "page_obj": page_obj,
            "search": search,
            "sort": sort,
...
def add_coupon(request):
    ...
    if request.method == "POST":
        ...
        errors, cleaned_data = validate_coupon(request.POST)
        if not errors:
    ...
    return render(
        request, "staff/add_coupon.html", {"active_page": "admin_coupon_management"}
    )
...
def edit_coupon(request, coupon_id):
    ...
    return render(
        ...
        {"active_page": "admin_coupon_management", "coupon": coupon},
    )
...
def sales_report(request):
    ...
    if search:
        orders = orders.filter(
            Q(order_id__icontains=search) | Q(user__name__icontains=search)
        )
    ...
    final_revenue = sum(order.final_amount for order in orders)
    paginator = Paginator(orders, 5)
    page_number = request.GET.get("page")
    page_obj = paginator.get_page(page_number)
    ...
    return render(
        ...
        "staff/sales_report.html",
        {
            ...
            "start_date_display": start.strftime("%d %b, %Y"),
...
def sales_report_pdf(request):
    ...
    orders = (
        Orders.objects.filter(created_at__date__gte=start, created_at__date__lte=end)
    ...
    # Title
    elements.append(Paragraph("Nova Style - Sales Report", styles["Title"]))
...
def admin_dashboard(request):
    ...
    if chart_type == "daily":
        sales_data = (
            delivered_orders.annotate(period=TruncDay("created_at"))
            .values("period")
        ...
        sales_data = (
            delivered_orders.annotate(period=TruncYear("created_at"))
            .values("period")
            .annotate(revenue=Sum("final_amount"))
            .order_by("period")
        )
        ...
        sales_data = (
            delivered_orders.annotate(period=TruncMonth("created_at"))
            .values("period")
            .annotate(revenue=Sum("final_amount"))
            .order_by("period")
        )
